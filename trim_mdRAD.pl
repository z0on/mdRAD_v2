...
- Filters mdRAD (v.2, with MspJI + MseI and no CC doublet in NNRW adaptor) 
fastq reads to leave only those proper degenerate tag at the 5' end (NNRW)
and on the 3' end - MseI site remnant (TA), secondary 3'-barcode, and 
Illumina adaptor (AGATCG). 
Trims away everything except the MspJI-MseI fragment itself (without the TA at 3'-end)
...
bc2=[perls-style barcode pattern] in-line barcode that follows the mdRAD fragment, 
		default \'[ATGC]{4}\'
adaptor=[far-end adaptor sequence] default AGATCG?
...
Example:
trim_mdRAD.pl input=Myproject_L8_G3.fastq sampleID=3 
...
while (<INP>) {
	...
	if ($ll==3 && $_=~/^(\@.+)$/) {
		...
		$counter++;
#print "$seq:";
		if ($seq=~/^($degen)(.+)TA($bcod)$adap/ and $seq!~/N/) {
			$read=$2;
